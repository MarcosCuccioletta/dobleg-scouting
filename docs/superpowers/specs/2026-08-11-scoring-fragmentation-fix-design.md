...
- **Síntomas 2 y 4 confirmados leyendo `recalc-scores/index.ts` línea por línea.** La función agrupa los partidos de un jugador por posición **dentro de cada liga por separado** (bucle `for (const league of leaguesForSeason)`, filas etiquetadas con `league_id: league.id`), y aunque después consolida a cada jugador en su posición primaria (`bestPos`, la de más partidos), esa consolidación solo compara el **string de posición**, no fusiona las filas: si un jugador tuvo partidos bajo dos `league_id` distintos en la misma posición esa temporada, quedan **dos filas separadas** con la misma posición pero distinto `matches_played`/`avg_score` (`player_season_scores` tiene PK `(player_id, season, position, league_id)` — el `league_id` extra es justamente lo que permite la duplicación). Ningún punto de lectura (ficha, Informes) las fusiona; cada uno elige "una" de las dos según el orden en que Postgres las devuelve, sin garantía de elegir la mejor.
  **Corrección post-implementación (revisión final):** el ejemplo original de esta spec decía "liga doméstica + copa", pero eso no fragmenta en este modelo de datos — `league_id` se deriva de `team_id` (`teams.league_id`), así que los partidos de copa de un mismo equipo comparten el mismo `league_id` que sus partidos de liga. El disparador real es un jugador que en la misma temporada jugó para equipos de **dos ligas distintas** (`has_player_stats = true` en ambas) — típicamente un pase o préstamo a mitad de temporada entre dos ligas que la plataforma sincroniza. Fusionar esas filas en una es la decisión correcta (evita el bug de duplicados), pero tiene una consecuencia consciente: `fetch_players_list` filtra por `league_id`, así que ese jugador deja de aparecer al filtrar Búsqueda de Talento por la liga "vieja" una vez fusionado — queda solo bajo la liga con más partidos jugados esa temporada. Trade-off aceptado, no un bug.
...
// temporada (típicamente un pase/préstamo a mitad de temporada entre dos
// ligas distintas que la plataforma sincroniza). Sin este paso quedan dos
