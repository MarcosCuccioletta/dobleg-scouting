  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <div className="bg-apple-gray-50 dark:bg-apple-gray-900/40 rounded-apple-lg p-4">
        <p className="text-2xs font-semibold text-apple-gray-400 uppercase tracking-wide mb-1">Eficacia en puntos</p>
        <p className="text-2xl font-bold text-brand-green">{fmtPct(pointsPct)}</p>
        <p className="text-2xs text-apple-gray-400 mt-0.5">{stats.points} de {stats.possiblePoints} puntos posibles</p>
      ...
      <div className="bg-apple-gray-50 dark:bg-apple-gray-900/40 rounded-apple-lg p-4">
        <p className="text-2xs font-semibold text-apple-gray-400 uppercase tracking-wide mb-1">% Victorias</p>
        <p className="text-2xl font-bold text-apple-gray-800 dark:text-white">{fmtPct(winPct)}</p>
        <p className="text-2xs text-apple-gray-400 mt-0.5">{stats.won} de {stats.played} partidos</p>
        {stats.played > 0 && (
      ...
      <div className="bg-apple-gray-50 dark:bg-apple-gray-900/40 rounded-apple-lg p-4 col-span-2 sm:col-span-1">
        <p className="text-2xs font-semibold text-apple-gray-400 uppercase tracking-wide mb-1">Puntos por partido</p>
        <p className="text-2xl font-bold text-apple-gray-800 dark:text-white">{ppg === null ? '–' : ppg.toFixed(2)}</p>
        <p className="text-2xs text-apple-gray-400 mt-0.5">Promedio sobre {stats.played} PJ</p>
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <span className="w-14 text-2xs text-apple-gray-500 dark:text-apple-gray-400 flex-shrink-0">Local</span>
        ...
        <div className="flex items-center gap-2">
          <span className="w-14 text-2xs text-apple-gray-500 dark:text-apple-gray-400 flex-shrink-0">Visitante</span>
          <div className="flex-1 h-2.5 rounded-full bg-apple-gray-200 dark:bg-apple-gray-700 overflow-hidden">
...
function HomeAwaySplit({ rows }: { rows: EnrichedMatchRow[] }) {
  ...
  return (
    <div className="bg-apple-gray-50 dark:bg-apple-gray-900/40 rounded-apple-lg p-4">
      <h4 className="text-xs font-semibold text-apple-gray-800 dark:text-white mb-3">Local vs. visitante</h4>
      <div className="space-y-3">
        <SplitBar label="Puntos por partido" home={home.ppg} away={away.ppg} digits={2} />
        <SplitBar label="% Victorias" home={home.winPct} away={away.winPct} isPct digits={0} />
      </div>
    </div>
  )
}
...
function CumulativePointsChart({ rows }: { rows: EnrichedMatchRow[] }) {
  ...
  return (
    <div className="bg-apple-gray-50 dark:bg-apple-gray-900/40 rounded-apple-lg p-4">
      <h4 className="text-xs font-semibold text-apple-gray-800 dark:text-white mb-2">Puntos acumulados</h4>
      <div className="h-40">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(value: unknown) => [String(value), 'Puntos']}
...
export default function CoachDtEfficiencyPanel({ rows, stats }: { rows: EnrichedMatchRow[]; stats: SeasonStats }) {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-apple-gray-800 dark:text-white">Eficacia del DT</h3>
