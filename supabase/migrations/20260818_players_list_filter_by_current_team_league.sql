-- Bug real reportado 2026-08-18, con capturas: filtrar Scout Externo por una liga
-- (ej. "Liga Profesional (Argentina)") mostraba jugadores que ya NO juegan ahí --
-- Keylor Navas (Pumas UNAM, México), Matías Perello (Independiente del Valle,
-- Ecuador) -- con su equipo actual mostrado correctamente (la columna EQUIPO sale
-- del join por current_team_id, que sí estaba al día), pero apareciendo igual bajo
-- el filtro de la liga vieja.
--
-- Causa raíz: el filtro `p_league_id` comparaba contra `pss.league_id` -- la liga
-- donde se jugaron ESOS partidos históricos -- en vez de contra la liga del club
-- ACTUAL del jugador. Un jugador que se fue de la Liga Profesional a mitad de
-- temporada sigue teniendo filas en `player_season_scores` con league_id=128 de
-- cuando jugaba ahí, y esas filas seguían pasando el filtro aunque el jugador ya
-- no esté en esa liga. Verificado contra datos reales: Navas y Perello tienen
-- ambos una fila 2025/liga=128 y una 2026/liga real de su club actual.
--
-- Fix: filtrar por la liga del equipo actual (`tm.league_id`, ya joineado por
-- current_team_id) en vez de por la liga de la fila de stats. Mismo criterio que ya
-- usa `p_team_id` (que sí filtra por `pl.current_team_id`, nunca tuvo este bug).
-- Un jugador sin equipo actual (agente libre) ahora no aparece filtrando por liga
-- -- correcto, ya no juega en ninguna.
...
AS $$
  ...
  by_player AS (
    ...
    FROM filtered
    ORDER BY player_id, (position = primary_position) DESC, matches_played DESC, season DESC, avg_score DESC
  ),
