-- Dos bugs reales encontrados en la auditoría del 2026-08-01, verificados contra
-- datos en producción:
--
-- 1) `by_player` elegía la fila de player_season_scores con más partidos jugados
--    SIN exigir que coincida con `primary_position` del jugador. ~6-7% de los
--    jugadores mostraban métricas/score de una posición secundaria mientras la
--    ficha/filtros asumían la primaria (ej. Messi con `primary_position=DEL` pero
--    mostrando su fila de VI de 5 partidos; Vidal con `primary=VC` mostrando CB).
--    Ahora se prioriza la fila que coincide con `primary_position`, y sólo se cae a
--    "la de más partidos" cuando el jugador no tiene ninguna fila en su posición
--    primaria (mismo criterio que ya usa fetchPlayerDetail() en
--    playerStatsService.ts).
--
-- 2) El filtro `p_max_contract_months` no tenía cota inferior: un contrato YA
--    VENCIDO (fecha pasada) también cumple "<= ahora + X meses" y aparecía
--    mezclado con los que realmente vencen pronto (~22% de una muestra real con
--    "vence en 6 meses" resultaron ser contratos ya vencidos, no próximos a
--    vencer).
