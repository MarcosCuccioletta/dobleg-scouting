-- fetch_players_list nunca devolvia el campo `league` (sólo `team.league_id`,
-- un número sin nombre). Toda página que lo consume (PlayerSearch de
-- ComparisonPage, los desplegables de liga de BusquedaPage) lo lee como
-- `player.league?.name` y siempre daba "—" — descubierto de paso al construir
-- Detector de Talentos, que necesita esto para el filtro de liga.
-- Se joinea `leagues` por pss.league_id (la liga en la que se calculó ESE
-- season_score, más preciso que el league_id del equipo actual si el jugador
-- se transfirió después del cierre de temporada).
